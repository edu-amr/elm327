...
async function runWithPort(port: string): Promise<void> {
  ...
  const foundPids: Array<{ pid: number; description: string }> = [];
  ...
  client.on('scanProgress', (data: { pid: number; response: any }) => {
  ...
  client.on('scanComplete', (data: { totalScanned: number; found: number; results: any }) => {
    console.log('');
    console.log('========================================');
